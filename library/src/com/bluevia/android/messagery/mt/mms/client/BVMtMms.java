...
public class BVMtMms extends BVMtMmsClient {
	...
	public String send(String destination, String subject, String message, 
		...
		//Convert attachments in parts
		ArrayList<BlueviaPartBase> parts = attachmentToParts(attachments);
		
		//Add message
		addMessageToAttachments(message, parts);

		return send(new MmsMessageReq(subject, new UserId(Type.PHONE_NUMBER, destination), parts, endpoint, correlator));
	}
	...
	public String send(ArrayList<String> destination, String subject, String message, ArrayList<Attachment> attachments,
		...
		return send(new MmsMessageReq(subject, addressList, parts, endpoint, correlator));
	}
