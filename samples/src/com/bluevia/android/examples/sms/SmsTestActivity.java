...
import com.bluevia.android.rest.messaging.sms.client.BlueviaSmsClient;
import com.bluevia.android.rest.messaging.sms.client.SmsClient;
import com.bluevia.android.rest.oauth.data.Token;

public class SmsTestActivity extends Activity {

	public static final String TAG = "SmsTestActivity";
	
	/*
	 * Change this constant to select other modes
	 */
	private static final Mode mMode = Mode.HTTP_SANDBOX;
	...
	private void createClient(){
		...
		Token aToken = new Token(OAUTH_TOKEN, OAUTH_TOKEN_SECRET);
		try {
			mClient = new BlueviaSmsClient(this,
					mMode,
					CONSUMER_KEY,
					CONSUMER_SECRET, aToken);
	...
	/**
	 * Note: the sendSms call is wrapped in an Async Task to avoid the UI being blocked by
	 * the HTTP request.
	 * 
	...
	private void sendSms(){
		...
		try {
			new SendSmsTask(this).execute(addresses, text, OAUTH_TOKEN);
	...
	/**
	 * Note: the getSmsDeliveryStatus call is wrapped in an Async Task to avoid the UI being blocked by
	 * the HTTP request.
	 * 
	 * Params: smsId
