...
public class BlueviaOauthClient extends AbstractRestClient{
	...
	protected static final String DEVELOPERS_ENDPOINT = "https://bluevia.com/test-apps/authorise?oauth_token=";
    protected static final String CONNECT_ENDPOINT = "https://connect.bluevia.com/authorise?oauth_token=";
	...
	public BlueviaOauthClient(Context context, AbstractRestClient.Mode mode, String consumerKey, String consumerSecret) throws BlueviaClientException {
        super(context, FEED_OAUTH_BASE_URI, 
        		isHttpMode(mode) ? new HttpConnector() : new BlueviaOauthMockConnector(mode.ordinal()), 
        				new Oauth(consumerKey, consumerSecret), new XmlOauthParserFactory());
	
        setAuthorizeEndpoint(mode);
	}
	...
	protected RequestToken getRequestToken(String callback, String apiName, Entity request) throws BlueviaClientException {
		...
		if (callback != null && callback.trim().length() != 0){
			if (!callback.equals(Oauth.CALLBACK) && !Utils.isNumber(callback))
				throw new BlueviaClientException("Invalid parameter: phoneNumber", BlueviaClientException.BAD_REQUEST_EXCEPTION);
		...
		RequestToken requestToken = new RequestToken((Token)response, mAuthorizeEndpoint);
     ...
     * @throws BlueviaClientException if an exception occurs during the request.
     */
	protected BlueviaOauthClient(Context context, Mode mode, IConnector connector, String consumerKey, String consumerSecret) throws BlueviaClientException {
        super(context, FEED_OAUTH_BASE_URI, connector, new Oauth(consumerKey,consumerSecret), new XmlOauthParserFactory());
	
        setAuthorizeEndpoint(mode);
	}
	
	private void setAuthorizeEndpoint(Mode mode){
        switch (mode) {
		case LIVE:
			mAuthorizeEndpoint = CONNECT_ENDPOINT;
			break;
		case TEST:
		case SANDBOX:
			mAuthorizeEndpoint = DEVELOPERS_ENDPOINT;
		break;
        default:
        	mAuthorizeEndpoint = CONNECT_ENDPOINT; //Mock modes
        break;
        }
	}
	///@endcond
