///@cond private
package com.bluevia.android.rest.location.parser.xml;
...
import com.bluevia.android.rest.location.data.LocationInfoType;
import com.bluevia.android.rest.location.data.LocationDataType.RetrievalStatusType;
 ...
 *  
 */
public class XmlLocationParser implements BlueviaEntityParser {
		...
		while (eventType != XmlPullParser.END_DOCUMENT){ 
			...
			if (eventType == XmlPullParser.START_TAG){
				if (tagName.equals(XmlConstants.XSD_TERMINALLOCATION_LOCATION)){
					...
					Entity entity = parseLocationEntity(eventType);
		...
		while (eventType != XmlPullParser.END_DOCUMENT){ 
			...
			case XmlPullParser.START_TAG:
				if (tagName.equals(XmlConstants.XSD_TERMINALLOCATION_LOCATEDPARTY)){
					...
					try{
						RetrievalStatusType status = stringToRetrievalStatus(XmlParserSerializerUtils.getChildText(mParser));
	...
	private RetrievalStatusType stringToRetrievalStatus(String typeStr) {
		if (typeStr == null)
			return null;
