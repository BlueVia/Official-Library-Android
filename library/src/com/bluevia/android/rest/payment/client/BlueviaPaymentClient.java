...
public class BlueviaPaymentClient extends AbstractRestClient {
	...
	public BlueviaPaymentClient(Context context, AbstractRestClient.Mode mode, String consumerKey, String consumerSecret) throws BlueviaClientException {
		super(context, (mode != Mode.HTTP_SANDBOX) ? FEED_PAYMENT_BASE_URI_BLUEVIA : FEED_PAYMENT_SANDBOX_BASE_URI, 
				(mode == Mode.HTTP || mode == Mode.HTTP_SANDBOX)? new HttpConnector() : new BlueviaPaymentMockConnector(mode.ordinal()), 
						new Oauth(consumerKey, consumerSecret), new BlueviaPaymentParserFactory());

		mOauthClient = new BlueviaPaymentOauthClient(context, mConnector, consumerKey, consumerSecret);
	 ...
	 * @throws BlueviaClientException if an exception occurs during the request.
	 */
	protected BlueviaPaymentClient(Context context, IConnector connector, String consumerKey, String consumerSecret) throws BlueviaClientException {
		...
		mOauthClient = new BlueviaPaymentOauthClient(context, connector, consumerKey, consumerSecret);
	}
	...
	private class BlueviaPaymentOauthClient extends BlueviaOauthClient {
		private BlueviaPaymentOauthClient(Context context, IConnector connector, String consumerKey, String consumerSecret) throws BlueviaClientException{
			super(context, connector, consumerKey, consumerSecret);
		}
